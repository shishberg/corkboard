...
describe('widgets', () => {
  it('CalendarWidget agenda shows only days with events, skipping empty ones', () => {
    const el: CalendarEl = { id: 'cal', type: 'calendar', variant: 'agenda', x: 0, y: 0, w: 300, h: 400, feedId: '', colour: 'black', font: 'atkinson-hyperlegible', align: 'center' as const, daysAhead: 7 as const }
  ...
  it('CalendarWidget applies element colour as text colour', () => {
    const el: CalendarEl = { id: 'cal', type: 'calendar', variant: 'agenda', x: 0, y: 0, w: 300, h: 200, feedId: '', colour: 'blue', font: 'atkinson-hyperlegible', align: 'center' as const, daysAhead: 7 as const }
    const w = mount(CalendarWidget, { props: { el } })
  ...
  it('CalendarWidget applies the element font as font-family', () => {
    const el: CalendarEl = { id: 'cal', type: 'calendar', variant: 'agenda', x: 0, y: 0, w: 300, h: 200, feedId: '', colour: 'black', font: 'gelasio', align: 'center' as const, daysAhead: 7 as const }
    const w = mount(CalendarWidget, { props: { el } })
    const style = w.find('[data-role="calendar-root"]').attributes('style') ?? ''
  ...
  it('CalendarWidget falls back to the default font when font is empty', () => {
    const el: CalendarEl = { id: 'cal', type: 'calendar', variant: 'agenda', x: 0, y: 0, w: 300, h: 200, feedId: '', colour: 'black', font: '', align: 'center' as const, daysAhead: 7 as const }
    const w = mount(CalendarWidget, { props: { el } })
    const style = w.find('[data-role="calendar-root"]').attributes('style') ?? ''
    expect(style).toContain('font-family: atkinson-hyperlegible')
  })

  it('CalendarWidget agenda variant renders bold day headings and event dividers', () => {
    const el: CalendarEl = { id: 'cal', type: 'calendar', variant: 'agenda', x: 0, y: 0, w: 300, h: 400, feedId: '', colour: 'black', font: 'atkinson-hyperlegible', align: 'center' as const, daysAhead: 7 as const }
    const w = mount(CalendarWidget, { props: { el } })
    const days = w.findAll('[data-role="day"]')
    expect(days.length).toBeGreaterThan(0)
    // Day headings are bold; events carry a 1px divider in the text colour.
    expect(days[0].attributes('style')).toContain('font-weight: 700')
    const events = w.findAll('[data-role="event"]')
    expect(events.length).toBeGreaterThan(0)
    expect(events[0].attributes('style')).toContain('border-bottom')
  })

  it('CalendarWidget agenda variant respects daysAhead (1 day shows one heading)', () => {
    const el: CalendarEl = { id: 'cal', type: 'calendar', variant: 'agenda', x: 0, y: 0, w: 300, h: 400, feedId: '', colour: 'black', font: 'atkinson-hyperlegible', align: 'center' as const, daysAhead: 1 as const }
    const w = mount(CalendarWidget, { props: { el } })
    // Only today is in range, so exactly one day heading ("Today") shows.
    const days = w.findAll('[data-role="day"]')
  ...
  it('CalendarWidget date variant shows the formatted sample date', () => {
    const el: CalendarEl = { id: 'cal', type: 'calendar', variant: 'date', x: 0, y: 0, w: 300, h: 200, feedId: '', colour: 'black', font: 'atkinson-hyperlegible', align: 'center' as const, daysAhead: 7 as const }
    const w = mount(CalendarWidget, { props: { el } })
