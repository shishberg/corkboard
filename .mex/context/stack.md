...
- **reka-ui** (shadcn-vue components) — base components live in `src/components/ui/` (Button, Popover, Tooltip). Prefer these over hand-rolling UI. The `cn()` helper is in `src/lib/utils.ts`. (Note: ToolRail currently hand-styles native `<button>`s for the icon rail — a known consistency nit, not shadcn `Button`.)
...
- *HTTP client (editor ↔ device)* — `src/lib/deviceApi.ts`, a `fetch`-based client for the endpoints in `context/protocol.md` (GET/PUT document, image upload, feeds, refresh, preview). Tolerant of an offline device. Built.
