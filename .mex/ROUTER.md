...
- Colour is one global current colour shown as a swatch panel below the tools in `ToolRail` (`colour` in toolOptions; `colour` on every `BaseEl`). It drives calendar/text glyphs and pen ink; selecting an element reflects its colour in the panel and clicking a swatch recolours it (`store.setElementColour`).
...
- 148 Vitest tests pass (`npm test`); `npm run build` clean. (Browser parity tests: `npm run test:parity`.)
...
- Still a stub: editor image-upload UI (`ImageOptions`) — the device `POST /api/images` endpoint works, but the editor doesn't call it yet (follow-up).
