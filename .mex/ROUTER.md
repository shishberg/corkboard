...
- Modules (`device/src/`): `main` (bootstrap + poll task — the poll task resolves once on startup
  ...
  day headings (Today, Tomorrow, then weekday names) with events indented beneath, 12-hour times, one
  shared font size, no boxes/dividers — mirrored exactly by `CalendarWidget.vue` for WYSIWYG. Feedless
