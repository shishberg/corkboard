...
The **web UI editor is fully migrated to the round-two model and working** (no device wiring yet) — editor surgery 1a–1d are all done (clock/timeline removed; `livePageId` + make-live + `deletePage`; calendar feed-reference; text element + bundled fonts). Element types: `calendar | image | drawing | text`. The device server is NOT built yet.
